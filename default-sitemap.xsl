...
>
			...
			<head>
				<title>
					<xsl:choose>
						...
						<xsl:otherwise>Índice del mapa del sitio</xsl:otherwise>
					</xsl:choose>
				</title>
	...
	<a href=""><span>Inicio</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<span>Índice del mapa del sitio</span>
	...
	<div id="content-head">
		...
		<p>Generado por <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, esto es un mapa del sitio XML, que se supone consumirán motores de búsqueda como Google o Bing</p>
		<p>
			Puede encontrar más información sobre mapas de sitio XML en <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						El mapa del sitio contiene						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						El índice de tu mapa del sitio contiene						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								mapa del sitio							</xsl:when>
							<xsl:otherwise>
								mapas del sitio							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				and was generated on enero 26, 2024 at 5:40 pm			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Baja</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">Media</xsl:when>
